...
def nearestOdd(n):
    if (roundHalfUp(n) % 2 ==0) & (n >roundHalfUp(n)):
...
def numberOfPoolBalls(rows):
    if rows > 0:
        return math.factorial(rows+1)/(math.factorial(rows-1)*2)
    else:
        return 0
...
def colorBlender(rgb1, rgb2, midpoints, n):
  if n<0 or n > (midpoints+1):
    ...
    y2 = rgb2 % 10**6 // 10**3
    y1 = rgb2 % 10**9 // 10**6
    a = roundHalfUp(x1 - ((x1-y1)/(midpoints+1)*n))*10**6
    b = roundHalfUp(x2 - ((x2-y2)/(midpoints+1)*n))*10**3
    c = roundHalfUp(x3 - (x3-y3)/(midpoints+1)*n)
    return a + b + c
...
def handToDice(hand):
    a, b, c = dice //100, dice //10 % 10, dice %10
    return a, b, c
