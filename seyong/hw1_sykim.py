...
def eggCartons(eggs):
    if eggs<0:
            print("양의 정수로 입력해주세요!")
            return None
    box=math.ceil(eggs/12)
    print(box)

def pascalsTriangleValue(row, col):
    if row<0 or col<0 or row<col:
        return None
    r = math.factorial(row)
    c = math.factorial(col)
    pascal_value=r/(c*math.factorial(row-col))
    print(pascal_value)
...
def nearestOdd(n):
    round = roundHalfUp(n)  # 주어진 숫자를 반올림하여 가장 가까운 정수로 변환
    if round % 2 == 0:  # 반올림된 값이 짝수인 경우
        return round - 1  # 가장 가까운 짝수는 반올림된 값에서 1을 더하면 됨
    else:  # 반올림된 값이 홀수인 경우
        return round  # 그대로 반환
...
def nearestOdd(n):
    round = roundHalfUp(n)  # 주어진 숫자를 반올림하여 가장 가까운 정수로 변환
    if round % 2 == 0:  # 반올림된 값이 짝수인 경우
        if n > round:  # 주어진 숫자가 반올림된 값보다 큰 경우
            return round + 1  # 가장 가까운 홀수는 반올림된 값보다 1 큼
        else:  # 주어진 숫자가 반올림된 값보다 작거나 같은 경우
            return round - 1  # 가장 가까운 홀수는 반올림된 값보다 1 작음
    else:  # 반올림된 값이 홀수인 경우
        return round  # 그대로 반환
...
def numberOfPoolBalls(rows):
    ...
    Balls_sum = rows*(rows+1)//2
    
    return Balls_sum

def numberOfPoolBallRows(balls):
    if type(balls) != int or balls < 0:
        print("음수가 아닌 int형태를 입력해주세용")
        return None
